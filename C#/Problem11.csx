...
int[][] grid = new int[][]{
    ...
    new int[]{78,17,53,28,22,75,31,67,15,94, 3,80, 4,62,16,14,09,53,56,92},
...
T[] Subarray<T>(T[] array, int index, int length)
{
...
int greatestProductInList(int[] values, int length)
{
    ...
    for(int i = 0; i < values.Length - length + 1; ++i)
    {
        int[] currentRun = Subarray(values,i,length);
        int product = 1;
        foreach(int value in currentRun)
        {
            product *= value;
        }
...
int greatestProductInGrid(int[][] grid, int length)
{
    List<int> products = new List<int>();
    foreach(int[] row in grid)
    {
        products.Add(greatestProductInList(row,length));
    }

    int[][] rotatedGrid = new int[grid.Length-1][];
    for(int i = 0; i < grid.Length; ++i)
    {
        for(int j = 0; i < grid.Length; ++j)
        {
            rotatedGrid[i] = new int [grid.Length-1];
            rotatedGrid[i][j] = grid[j][i];
            rotatedGrid[j][i] = grid[i][j];
        }
    }
    foreach(int[] row in rotatedGrid)
    {
        products.Add(greatestProductInList(row,length));
    }
...
Console.WriteLine(greatestProductInGrid(grid,4));